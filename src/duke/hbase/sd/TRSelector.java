...
public class TRSelector {
    ...
    public TRSelector() throws Exception{
      /*try{
	    cm_read = new CMDProxy("cm-1.2/bin/cm.sh", "read_training_data.csv");
	    cm_write = new CMDProxy("cm-1.2/bin/cm.sh", "write_training_data.csv");
	    cm_update = new CMDProxy("cm-1.2/bin/cm.sh", "update_training_data.csv");
	    cm_scan = new CMDProxy("cm-1.2/bin/cm.sh", "scan_training_data.csv");
	    cm_join = new CMDProxy("cm-1.2/bin/cm.sh", "join_training_data.csv");
	    bw_read = cm_read.GetInputWriter();
	    bw_write = cm_write.GetInputWriter();
	    bw_update = cm_update.GetInputWriter();
	    bw_scan = cm_scan.GetInputWriter();
	    bw_join = cm_join.GetInputWriter();    
       }
       catch(Exception e) {
         throw new Exception("Error: while starting the cost model");
       }*/
    }
